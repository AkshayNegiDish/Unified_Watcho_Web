...
export class MovieDetailPageComponent implements OnInit {
  ...
  constructor(private kalturaAppService: KalturaAppService, public kalturaUtilService: KalturaUtilService,
    private appUtilService: AppUtilService, private modalMessageService: ModalMessageService,
    private route: ActivatedRoute, private titleService: Title, private router: Router, private meta: Meta, @Inject(PLATFORM_ID) private platformId) {
  ...
  ngOnInit() {
    if (this.isBrowser) {
      this.DMS = this.appUtilService.getDmsConfig('movie');
      this.screenId = environment.ENVEU.SCREEN_IDS.MOVIE_DETAIL;;
  ...
  getAssetDetails(assetId: string) {
    ...
    this.kalturaAppService.getMediaAssetById(assetId).then((res: any) => {
      ...
      this.titleService.setTitle('Watch ' + this.assetDetails.name + ' full movie online on ' + AppConstants.APP_NAME_CAPS + ' | ' + AppConstants.APP_NAME_CAPS + ' Movies')
    }, reject => {
