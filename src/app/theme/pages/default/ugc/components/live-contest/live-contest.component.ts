...
@Component({
  ...
  styleUrls: ['./live-contest.component.scss']
})
...
export class LiveContestComponent implements OnInit {
  ...
  constructor(private ugcFormService: UgcFormService, private notificationService: NotificationService, private appUtilService: AppUtilService,
    private snackbarUtilService: SnackbarUtilService, private activatedRoute: ActivatedRoute, private modalService: NgbModal, private kalturaAppService: KalturaAppService, private router: Router) {
    this.contestModal = {
      id: null,
      name: null,
      description: null,
  ...
  participateNow(contestId: any) {
    if (this.appUtilService.isUserLoggedIn()) {
      this.router.navigate(['/ugc/my-uploads'], { queryParams: { "contestId": contestId } })
    } else {
